                    ...
                    <table class="table table-head-custom table-vertical-center table-head-bg table-borderless">
                      ...
                      <template v-else-if="mapCollection.length > 0">
                        <tr v-for="item in mapCollection">
                          <td class="pl-7 py-8">
                            <router-link :to="{name: 'trigger.map.update', params: {map: item.id}}"
                          ...
                          <td>
                            <router-link v-if="item.project"
                                         :to="{name: 'marketing.project.update', params: {project: item.project.id}}"
                                         class="text-dark-75 font-weight-bolder text-hover-primary d-block font-size-lg">
                              {{ item.project.name }}
                            </router-link>
                            <span v-else
                                  class="text-dark-75 font-weight-bolder d-block font-size-lg"> - template - </span>
                          </td>
                          <td>
                            <div class="mb-3">
                              <span v-if="item.is_enabled"
                                    class="label label-lg label-inline font-weight-bold label-light-primary text-uppercase">Enabled</span>
                              <span v-else
                                    class="label label-lg label-inline font-weight-bold label-light-danger text-uppercase">Disabled</span>
                            </div>

                            <span class="font-weight-bold"><span class="text-muted">Delay:</span> <span
                                class="text-dark">{{ item.shutdown_delay }} seconds</span></span>
                          </td>
                          <td class="pr-0 text-right">
                            <router-link :to="{name: 'trigger.map.update', params: {map: item.id}}"
                                         class="btn btn-light-success font-weight-bolder font-size-sm">View Trigger Map
                            </router-link>
                          </td>
                        </tr>
                      </template>
                    ...
                    <table class="table table-head-custom table-vertical-center table-head-bg table-borderless">
                      ...
                      <template v-else-if="projectCollection.length > 0">
                        <tr v-for="item in projectCollection">
                          ...
                          <td>
                            <router-link v-if="item.organization"
                                         :to="{name: 'marketing.organization.update', params: {organization: item.organization.id}}"
                                         class="text-dark-75 font-weight-bolder text-hover-primary d-block font-size-lg">
                          ...
                          <td>
                            <span v-if="item.account" class="text-dark-75 font-weight-bolder d-block font-size-lg">{{
                                item.account.email
                              }}</span>
                            <span v-else class="text-dark-75 font-weight-bolder d-block font-size-lg"> - unknown account - </span>

                            <span v-if="item.account" class="text-muted font-weight-bold">{{
                          ...
                          <td>
                            ...
                            <span class="font-weight-bold">
                              ...
                              <template v-if="item.date_start_at">{{ item.date_start_at|date }}</template><template
                                v-else>not set</template>
                    ...
                    <table class="table table-head-custom table-vertical-center table-head-bg table-borderless">
                      ...
                      <template v-else-if="campaignCollection.length > 0">
                        <tr v-for="item in campaignCollection">
                          <td class="pl-7 py-8">
                            <router-link :to="{name: 'marketing.campaign.update', params: {campaign: item.id}}"
                                         class="text-dark-75 font-weight-bolder text-hover-primary mb-1 font-size-lg">
                              {{ item.name }}
                            </router-link>
                            <span class="text-muted font-weight-bold d-block">{{
                                item.foreign_campaign_id|adwordsID
                              }}</span>
                          </td>
                          <td>
                            <router-link v-if="item.map"
                                         :to="{name: 'marketing.map.update', params: {map: item.map.id}}"
                                         class="text-dark-75 font-weight-bolder text-hover-primary d-block font-size-lg">
                              {{ item.map.name }}
                            </router-link>
                          ...
                          <td class="pr-0 text-right">
                            <router-link :to="{name: 'marketing.campaign.update', params: {campaign: item.id}}"
                                         class="btn btn-light-success font-weight-bolder font-size-sm">View Campaign
                            </router-link>
                          </td>
                        </tr>
                      </template>
